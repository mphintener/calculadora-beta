...
with st.form("beta_calc"):
    ...
    with col2: vida = st.number_input("🏠 CUSTO VIDA:", min_value=0.0, help="Opcional: Gastos fixos (moradia, luz, alimentação).")
...
if btn and sal > 0:
    ...
    # MAPA COM LINHA SUAVE (MENOS "ESQUISITO")
    ...
    st.pydeck_chart(pdk.Deck(
        initial_view_state=pdk.ViewState(
            ...
            zoom=11, pitch=0 # Mapa plano para evitar distorção do arco
        ),
        layers=[
            pdk.Layer(
                "LineLayer", data=line_data, get_source_position=["slon", "slat"],
                get_target_position=["elon", "elat"],
                get_color=[255, 204, 0, 255], get_width=15
            ),
            pdk.Layer(
                "ScatterplotLayer", data=line_data, get_position=["slon", "slat"],
                get_color=[255, 204, 0], get_radius=300
            ),
            pdk.Layer(
                "ScatterplotLayer", data=line_data, get_position=["elon", "elat"],
                get_color=[230, 57, 70], get_radius=300
            )
        ]
    ))

    # SÍNTESE COM ÊNFASE TOTAL E GASTO MENSAL
