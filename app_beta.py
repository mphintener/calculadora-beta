...
# BASE DE DADOS GEO COMPLETA (DISTRITOS + RMSP)
# Adicionei a lista massiva solicitada com coordenadas aproximadas para garantir o arco
geo_db = {
...
with st.form("beta_calc"):
    ...
    with col2: vida = st.number_input("🏠 CUSTO VIDA:", min_value=0.0, help="Preenchimento Opcional: Gastos fixos (moradia, luz, alimentação).")
...
if btn and sal > 0:
    h_paga = 176
    custo_t_mes = (p_pub + p_app + p_car) * dias_presenca
    h_total_exprop = h_trecho * dias_presenca
    v_hora_real = (sal - custo_t_mes) / (h_paga + h_total_exprop)
    perda = (1 - (v_hora_real / (sal/h_paga))) * 100
    sobra_final = sal - custo_t_mes - vida
    ...
    # MAPA COM ARCO REFINADO
    st.markdown('<div class="secao-titulo">🗺️ ARCO DE EXPROPRIAÇÃO ESPACIAL</div>', unsafe_allow_html=True)
    arc_df = pd.DataFrame([{
        "slat": geo_db[moradia][0], "slon": geo_db[moradia][1],
        "elat": geo_db[trabalho][0], "elon": geo_db[trabalho][1]
    }])
    
    # Zoom dinâmico baseado na distância
    st.pydeck_chart(pdk.Deck(
        initial_view_state=pdk.ViewState(
            latitude=(geo_db[moradia][0] + geo_db[trabalho][0]) / 2,
            longitude=(geo_db[moradia][1] + geo_db[trabalho][1]) / 2,
            zoom=11 if moradia != trabalho else 13, pitch=45
        ),
        layers=[
            pdk.Layer(
                "ArcLayer", data=arc_df, get_source_position=["slon", "slat"],
                get_target_position=["elon", "elat"],
                get_source_color=[255, 204, 0, 200], get_target_color=[230, 57, 70, 200],
                get_width=12, tilt=15 # Curvatura ajustada para trajetos curtos
            )
        ]
    ))
    ...
    # SÍNTESE COM ÊNFASE NO GASTO MENSAL
    ...
    st.markdown(f"""
        <div class="sintese-box">
            ...
            Ao se deslocar {local_txt}, você gasta <span style="color:#FFCC00">R$ {custo_t_mes:,.2f}</span> mensais com transporte e dedica <span style="color:#FFCC00">{h_total_exprop:.0f} horas</span> não remuneradas ao sistema.<br><br>
