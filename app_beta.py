...
st.markdown("""
    ...
    .sintese-box { background-color: #111; border-left: 10px solid #E63946; border-right: 2px solid #333; padding: 40px; margin-top: 50px; color: #FFFFFF; font-size: 1.4rem; line-height: 1.5; font-weight: 600; }
    </style>
    """, unsafe_allow_html=True)

# 1. BASE GEOGRÁFICA MASSIVA (DISTRITOS + RMSP COMPLETA)
geo_db = {
    "São Paulo (Centro)": [-23.5505, -46.6333], "Barra Funda": [-23.5255, -46.6669], "Pinheiros": [-23.5611, -46.7011],
    "Lapa": [-23.5222, -46.7028], "Santana": [-23.5015, -46.6233], "Itaim Bibi": [-23.5833, -46.6833],
    "Butantã": [-23.5711, -46.7081], "Grajaú": [-23.7744, -46.6975], "Itaquera": [-23.5333, -46.4583],
    ...
    "Taboão da Serra": [-23.6256, -46.7575], "Barueri": [-23.5112, -46.8761]
}
# Nota: Adicione os outros 80+ distritos aqui conforme sua lista oficial
...
with st.form("beta_calc"):
    ...
    with col2: 
        vida = st.number_input("🏠 CUSTO VIDA:", min_value=0.0, help="Preenchimento Opcional: Gastos fixos (aluguel, contas, alimentação).")
...
if btn and sal > 0:
    ...
    point_df = pd.DataFrame([
        {"pos": [geo_db[moradia][1], geo_db[moradia][0]], "color": [255, 204, 0], "radius": 400},
        {"pos": [geo_db[trabalho][1], geo_db[trabalho][0]], "color": [230, 57, 70], "radius": 400}
    ])
    ...
    st.pydeck_chart(pdk.Deck(
        ...
        layers=[
            # Linha de Desejo (Conexão técnica)
            pdk.Layer(
                "LineLayer", data=line_df, get_source_position="source", get_target_position="target",
                get_color=[255, 204, 0, 100], get_width=10
            ),
            # Marcadores de "Impacto Geográfico"
            pdk.Layer(
                "ScatterplotLayer", data=point_df, get_position="pos", get_color="color",
                get_radius="radius", opacity=0.6, stroked=True, line_width_min_pixels=2
            )
        ]
    ))

    # SÍNTESE FINAL (ESTILO SÓBRIO)
    ...
    st.markdown(f"""
        <div class="sintese-box">
            <b>SÍNTESE DA EXPROPRIAÇÃO ({dias_presenca} DIAS):</b><br><br>
            ...
            O seu <b>VALOR REAL PELA HORA DE TRABALHO PAGA</b> é de <b>R$ {max(0, v_hora_real):.2f}</b>.{sintese_v}
        </div>
    """, unsafe_allow_html=True)
