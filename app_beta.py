...
# CSS: ESTILO DE ALTO IMPACTO (PRETO, AMARELO E VERMELHO)
st.markdown("""
    <style>
    ...
    .card-res { background-color: #111; border: 2px solid #FFCC00; padding: 20px 10px; text-align: center; border-radius: 5px; min-height: 150px; display: flex; flex-direction: column; justify-content: center; }
    .val-res { color: #FFCC00 !important; font-size: 2rem !important; font-weight: 900 !important; margin-top: 10px; }
    .label-card { color: #FFFFFF !important; font-size: 0.7rem !important; font-weight: bold; text-transform: uppercase; line-height: 1.2; }
    .sintese-box { background-color: #E63946; border: 4px solid #FFCC00; padding: 30px; margin-top: 40px; color: #FFFFFF; font-size: 1.3rem; line-height: 1.6; font-weight: 700; border-radius: 10px; text-align: center; }
    </style>
    """, unsafe_allow_html=True)

# 1. BASE DE DADOS COMPLETA (DISTRITOS + MUNICÍPIOS) COM COORDENADAS
# Centralizei os pontos para garantir que o mapa sempre mostre algo na Grande SP
geo_db = {
...
if btn and sal > 0:
    ...
    # CARDS DE RESULTADO (TÍTULOS ATUALIZADOS)
    ...
    # MAPA ROBUSTO (CENTRALIZADO NA GRANDE SP)
    st.markdown('<div class="secao-titulo">🗺️ MAPEAMENTO DO DESLOCAMENTO NA RMSP</div>', unsafe_allow_html=True)
    ...
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', font_color='white', height=450, showlegend=False)
    st.plotly_chart(fig, use_container_width=True)

    # SÍNTESE COM ÊNFASE MÁXIMA
    ...
    st.markdown(f"""
        <div class="sintese-box">
            ...
            Ao se deslocar {local_txt}, você dedica <span style="color:#FFCC00">{h_total_exprop:.0f} horas</span> mensais não remuneradas ao sistema de mobilidade urbana.<br><br>
            Na prática, o seu <b>VALOR REAL PELA HORA DE TRABALHO PAGA</b> é de apenas <b>R$ {max(0, v_hora_real):.2f}</b>.{sintese_v}
        </div>
    """, unsafe_allow_html=True)
